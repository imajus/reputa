...
const ScoreReview = () => {
  ...
  const getTier = (score: number) => {
    if (score >= 800)
      return { name: "Premium", color: "text-primary", bg: "bg-primary/10" };
    if (score >= 600)
      return { name: "Standard", color: "text-chart-3", bg: "bg-chart-3/10" };
  ...
  const breakdownItems = [
    ...
    {
      label: "Protocol & Token Diversity",
      value: scoreBreakdown.diversity,
      icon: Layers,
    },
    {
      label: "Financial Health",
      value: scoreBreakdown.riskBehavior,
      icon: Shield,
    },
    {
      label: "Intent Alignment",
      value: scoreBreakdown.surveyMatch,
      icon: Target,
    },
  ];
          ...
          <CardContent className="space-y-8">
            ...
            <div className="rounded-lg bg-primary/5 p-4">
              <p className="text-sm text-muted-foreground">
                ...
                You qualify for {tier.name} tier benefits on Sui protocols,
                including enhanced APY rates and priority access to new
                features.
              </p>
            </div>
